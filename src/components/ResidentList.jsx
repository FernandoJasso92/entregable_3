...
const ResidentList = ({ location }) => {
  ...
  return (
    <>
      <section className=" p-4 grid gap-6 auto-rows-auto grid-cols-[repeat(auto-fill,_minmax(220px,_1fr))] max-w-[1000px] mx-auto bg-[url('/images/ellipse.png')] bg-cover">
        {residents?.slice(startCut, endCut).map((resident) => (
          <ResidentCard key={resident} resident={resident} />
        ))}
      </section>

      <ul className="flex gap-4 justify-center py-4 text-white font-bold">
        {arrayPages.map((page) => (
          <li
